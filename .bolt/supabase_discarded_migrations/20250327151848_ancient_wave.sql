/*
  ...
  1. Changes
    ...
    - Remove agency/customer_success roles
...
DO $$ BEGIN
  CREATE TYPE user_role_type AS ENUM ('creator', 'brand', 'admin');
EXCEPTION
...
SET role_new = CASE 
  ...
  ELSE 'brand'::user_role_type -- Convert any other roles to brand
END;
...
-- Update workspace creation policy to allow brands to create workspaces
CREATE POLICY "Brands can create workspaces"
  ON workspaces
  FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM users
      WHERE id = auth.uid()
      AND (role = 'brand'::user_role_type OR role = 'admin'::user_role_type)
    )
  );